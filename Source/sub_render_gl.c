    ...
    if (frame && frame->events) {
        ...
        while (ev) {
            if (ev->kind == SUB_EVENT_BITMAP && ev->data.bitmap.rgba) {
                ...
                // Evade the Android UI by subtracting the offset directly!
                int src_y = ev->y - (int)r->vertical_offset;
...
void sub_render_gl_set_vertical_offset(SUB_RENDERER *r, float offset) {
    if (!r) return;
    pthread_mutex_lock(&r->lock);
    r->vertical_offset = offset;
    pthread_mutex_unlock(&r->lock);
}

// ... your existing sub_render_gl_fill_bitmap function is right below this ...
